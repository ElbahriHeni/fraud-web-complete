...
const copy = {
  en: {
    title: 'New Case',
    subtitle: 'Create a fraud case using the updated seven-step wizard.',
    submit: 'Submit Case',
  ...
  ar: {
    title: 'بلاغ جديد',
    subtitle: 'إنشاء بلاغ احتيال باستخدام المعالج الجديد المكوّن من سبع خطوات.',
    submit: 'إرسال البلاغ',
...
export default function NewCaseWizardPage() {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    caseStatus: isArabic ? 'مسودة' : 'Draft',
    insuranceType: isArabic ? 'مركبات' : 'Motor',
    hasClaim: false,
    description: '',
    suspectedAmount: '',
    ...
    claimNumber: '',
    claimType: '',
    claimStatus: isArabic ? 'مفتوح' : 'Open',
    suspensionDate: '',
    suspensionReason: '',
    ...
    fraudOfficerDecision: isArabic ? 'فتح' : 'Open',
    assignedUser: '',
    fraudUnitNotes: '',
    closureReason: '',
  });
  ...
  const stepHelperText = isArabic
    ? 'تنقّل بين الخطوات، ثم احفظ كمسودة أو أرسل البلاغ كمفتوح عند الانتهاء.'
    : 'Move between the steps, then save as draft or submit as open when finished.';
  ...
  const handleSubmit = async (targetStatus: 'Draft' | 'Open') => {
    try {
      ...
      const createdCase = await apiPost<CreateCaseResponse>('/api/cases', {
        ...
        claim_status: form.hasClaim ? mapValue(form.claimStatus, maps.claimStatus) : '',
        suspension_date: form.hasClaim ? form.suspensionDate || null : null,
        suspension_reason: form.hasClaim ? form.suspensionReason : '',
    ...
    <div className="card form-grid two-col-form">
      ...
      <label style={fieldStyle}>
        ...
        <select value={form.caseSource} onChange={(e) => updateField('caseSource', e.target.value)}>
          {(isArabic ? ['الموقع', 'أخرى'] : ['Website', 'Other']).map((option) => <option key={option}>{option}</option>)}
        </select>
      </label>
      ...
      <label style={fieldStyle}>
        ...
        <input value={form.caseStatus} readOnly />
      </label>
      <label style={fieldStyle}>
        ...
        <select value={form.insuranceType} onChange={(e) => updateField('insuranceType', e.target.value)}>
          {(isArabic ? ['مركبات', 'طبي', 'حياة', 'عام'] : ['Motor', 'Medical', 'Life', 'General']).map((option) => <option key={option}>{option}</option>)}
        </select>
      </label>
      <label style={fieldStyle}>
  ...
  const renderClaimStep = () => (
    <div className="card form-grid two-col-form">
      ...
      <label style={fieldStyle}>
        <span>{isArabic ? 'حالة المطالبة' : 'Claim Status'}</span>
        <select value={form.claimStatus} onChange={(e) => updateField('claimStatus', e.target.value)}>
          {(isArabic ? ['مفتوح', 'معلق'] : ['Open', 'Suspended']).map((option) => <option key={option}>{option}</option>)}
        </select>
      </label>
      <label style={fieldStyle}>
      ...
      <div className="actions-inline" style={{ justifyContent: 'space-between', marginTop: 20 }}>
        ...
        {currentStepIndex < stepOrder.length - 1 ? (
          ...
          <div className="actions-inline" style={{ gap: 12, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            <button type="button" className="btn primary" onClick={() => handleSubmit('Open')} disabled={isSubmitting}>
